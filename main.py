...
estado_selecionado = st.selectbox(
    "Selecione o estado:",
    options=list(ESTADOS_CIDADES.keys())
)

# --- Multiselect de Cidades dependente do estado ---
cidades_disponiveis = ESTADOS_CIDADES.get(estado_selecionado, [])
cidades_escolhidas = st.multiselect(
    "Selecione a(s) cidade(s):",
    options=cidades_disponiveis,
    default=[cidades_disponiveis[0]] if cidades_disponiveis else [],  # Seleciona a primeira cidade como default
    max_selections=len(cidades_disponiveis)
)
...
# === LOOP PARA CADA CIDADE SELECIONADA ===
